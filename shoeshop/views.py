...
def update_customer_view(request, pk):
    ...
    return render(
        request,
        'shoeshop/admin_update_customer.html',
        context=mydict)
...
def admin_add_product_view(request):
    ...
    return render(request,
                  'shoeshop/admin_add_products.html',
...
def admin_add_category(request):
    ...
    return render(request,
                  'shoeshop/admin_add_category.html',
...
def update_product_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if productForm.is_valid():
            ...
            return redirect('admin-products')
    return render(request,
                  'shoeshop/admin_update_product.html',
                  {'productForm': productForm})
...
def admin_view_booking_view(request):
    ...
    for order in orders:
        ...
        ordered_products.append(ordered_product)
        ordered_bys.append(ordered_by)
    return render(request, 'shoeshop/admin_view_booking.html',
                  {'data': zip(ordered_products, ordered_bys, orders)})
...
def brand_show(request):
    """Show brands for admin"""
    brand_name = models.Brand.objects.all()
    return render(request,
                  '/shoeshop/homebase.html',
                  {'brand_name': brand_name})
...
def customer_address_view(request):
    ...
    if request.method == 'POST':
        ...
        if addressForm.is_valid():
            ...
            # for showing total price on payment page.....accessing id from
            # cookies then fetching  price of product from db
            total = 0
            if 'product_ids' in request.COOKIES:
                product_ids = request.COOKIES['product_ids']
                if product_ids != "":
            ...
            return response
    return render(request,
                  'shoeshop/customer_address.html',
...
def payment_success_view(request):
    ...
    # after that we will delete cookies because after order placed...cart
    # should be empty
    ...
    if 'product_ids' in request.COOKIES:
        ...
        if product_ids != "":
            product_id_in_cart = product_ids.split('|')
            products = models.Product.objects.all().filter(id__in=product_id_in_cart)
            # Here we get products list that will be ordered by one customer at
            # a time
    ...
    # there will be lot of redundant data in orders table...but its become
    # more complicated if we normalize it
...
def my_order_view(request):
    ...
    return render(request, 'shoeshop/my_order.html',
